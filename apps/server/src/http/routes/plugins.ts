...
export const pluginRoutes = (deps: AppDeps) =>
  new Elysia({ prefix: "/api/plugins" })
    ...
    .post("/register", async ({ body }) => {
      ...
      const modulePath =
        typeof payload.modulePath === "string" ? payload.modulePath : "";
    ...
    .put("/:id/permissions", async ({ params, body }) => {
      ...
      const next = deps.pluginManager.updatePermissions(params.id, {
        ...
        replyGroup:
          typeof payload.replyGroup === "boolean" ? payload.replyGroup : undefined,
        configRead:
          typeof payload.configRead === "boolean" ? payload.configRead : undefined,
        configWrite:
          typeof payload.configWrite === "boolean" ? payload.configWrite : undefined
      });
